...
void LCD::setDmgPalette(PixelType *const palette, const PixelType *const dmgColors, const unsigned data) {
...
static unsigned long gbcToRgb16(const unsigned bgr15) {
	...
	if(useFullColorSaturation)
		return (r << 11) | ((unsigned int)(g * (63./31.)) << 5) | b;
	else
...
static unsigned long mapGbcColor(const unsigned bgr15) {
	if(usingRGB565Color)
		return gbcToRgb16(bgr15);
	else
	...
	m1IrqStatReg_(0)
{
	...
	for (std::size_t i = 0; i < sizeof(dmgColorsRgb) / sizeof(dmgColorsRgb[0]); ++i)
...
void LCD::refreshPalettes() {
	if (ppu.cgb()) {
		for (unsigned i = 0; i < 8 * 8; i += 2) {
			ppu.bgPalette()[i >> 1] = mapGbcColor( bgpData[i] |  bgpData[i + 1] << 8);
			ppu.spPalette()[i >> 1] = mapGbcColor(objpData[i] | objpData[i + 1] << 8);
		}
	} else {
		setDmgPalette(ppu.bgPalette()    , dmgColorsRgb    ,  bgpData[0]);
		setDmgPalette(ppu.spPalette()    , dmgColorsRgb + 4, objpData[0]);
		setDmgPalette(ppu.spPalette() + 4, dmgColorsRgb + 8, objpData[1]);
	}
}
...
void LCD::updateScreen(const bool blanklcd, const unsigned long cycleCounter) {
	...
	if (blanklcd && ppu.frameBuf().fb()) {
		const unsigned long color = ppu.cgb() ? mapGbcColor(0xFFFF) : dmgColorsRgb[0];
...
static void doCgbColorChange(unsigned char *const pdata,
		PixelType *const palette, unsigned index, const unsigned data) {
	pdata[index] = data;
	index >>= 1;
	palette[index] = mapGbcColor(pdata[index << 1] | pdata[(index << 1) + 1] << 8);
}
...
void LCD::setVideoBuffer(PixelType *const videoBuf, const int pitch) {
	ppu.setFrameBuf(videoBuf, pitch);
}

void LCD::setDmgPaletteColor(const unsigned index, const unsigned long rgb32) {
	if(usingRGB565Color) // convert to RGB565
		dmgColorsRgb[index] = ((rgb32 & 0xFF) >> 3) | (((rgb32 & 0xFF00) >> (8+2)) << 5) | (((rgb32 & 0xFF0000) >> (16+3)) << 11);
	else
		dmgColorsRgb[index] = rgb32;
}
